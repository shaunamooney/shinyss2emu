...
plot_emu_data <- function(emu_data, mcpr_data){
  ...
  if(nrow(mcpr_data) == 0){
    ...
    emu_plot <- ggplot() +
      ...
      theme_bw() +
      labs(x = x_axis, y = y_axis, colour = colour_type, linetype = " ", caption = emu_plot_caption) + ggtitle(emu_plot_title) + theme(legend.position = "top")
    ...
    delta_emu_plot <- ggplot() +
      ...
      theme_bw() + labs(x = x_axis, colour = colour_type, y = "Rate of Change", linetype = " ", shape = " ", caption = delta_plot_caption)  + ggtitle("Annual changes in EMU over time") + theme(legend.position = "top")
    ...
    emu_plot <- ggplot() +
      ...
      geom_errorbar(emu_data, mapping = aes(x = year, ymin = emu - sd_emu, ymax = emu + sd_emu, colour = emu_w_uncertainty), width = 0.5) +
      theme_bw() +
      labs(x = x_axis, y = y_axis, colour = " ", linetype = " ", caption = emu_plot_caption) +
      ggtitle(emu_plot_title) + theme(legend.position = "top") + guides(linetype = "none")

    # plotting delta EMU data
    delta_emu_plot <- ggplot() +
      geom_point(emu_data, mapping = aes(year, emu_roc, colour = emu_colour_delta)) +
      geom_errorbar(emu_data, mapping = aes(x = year, ymin = emu_roc - sd_emu_roc, ymax = emu_roc + sd_emu_roc, colour = emu_colour_delta), width = 0.5) +
